...
from utils import *
from unet import *
from train_AGT_defense_model import Clean
...
def main(Reload):
    ...
    for epoch in range(config.epochs):
        ...
        if test_acc > config.best_acc:
            tsm_load_path = os.path.join(model_save_path, f'tsm_{config.AGT_num}_{test_acc:.4f}.pt')
            pre_path = tsm_load_path.replace(f'{test_acc:.4f}', f'{config.best_acc:.4f}')
            if os.path.exists(pre_path) and pre_path is not config.tsm_load_path:
                os.remove(pre_path)
            torch.save(tsm_model.state_dict(), tsm_load_path)
...
def parse_args():
    ...
    parser.add_argument('--tsm_save_path', type=str, default=r'TSM')
...
if __name__ == '__main__':
    ...
    if config.dataset_name == 'cifar10':
        ...
        model_save_path = os.path.join(config.tsm_save_path, 'cifar10')
    else:
        num_classes = 100
        ...
        model_save_path = os.path.join(config.tsm_save_path, 'miniimagenet')
    ...
    tsm_load_path = config.tsm_load_path
    ...
    def_model.load_state_dict(torch.load(config.def_model_load_path, map_location=device))
    def_model.to(device)
    def_model.eval()
    ...
    for reload in range(config.reload):
        ...
        if tsm_load_path != '':
            tsm_model.load_state_dict(torch.load(tsm_load_path, map_location=device))
        tsm_model.to(device)
        print(f'Total params: {sum(p.numel() for p in tsm_model.parameters()) / 1000000.0:.2f}M')
        optimizer = optim.SGD(tsm_model.parameters(), config.lr, config.momentum, weight_decay=config.weight_decay)
